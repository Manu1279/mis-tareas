...
async function createCalendarEvent(task) {
  ...
  const event = {
    summary: task.title,
    description: `Prioridad: ${task.prior} | Categoria: ${task.cat}${task.deadline ? ' | Fecha limite: ' + fmt(task.deadline) : ''}`,
...
async function sendWeeklyReport() {
  ...
  try {
    ...
    alert('Error al codificar el mensaje: ' + e.message);
    return;
  }
...
function buildEmailBody(r) {
  ...
  return `
    ...
    <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%;margin:1rem 0">
      ...
      <tr><td>${r.done}</td><td>${r.pendingCount}</td><td>${r.overdue}</td><td>${r.avgShift} dias</td></tr>
    </table>
    ${r.pending.length ? `<h3>Tareas pendientes</h3>
    <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">
      <tr style="background:#E1F5EE"><th>Tarea</th><th>Categoria</th><th>Prioridad</th><th>Fecha limite</th></tr>
      ${rows}
    </table>` : '<p>No hay tareas pendientes!</p>'}
  `;
}
...
async function addTask() {
  ...
  if (!title) { alert('Escribi un titulo para la tarea.'); return; }
  const task = {
    id: Date.now(),
    title,
  ...
  if (accessToken && task.doDate && task.startTime && task.endTime) {
    ...
    status.textContent = 'Conecta Google para crear el evento automaticamente.';
    setTimeout(() => { status.textContent = ''; }, 4000);
  }
}
...
function render() {
  ...
  else if (filterVal === 'familia')     list = tasks.filter(t => t.cat === 'familia');
  else if (filterVal === 'personal')    list = tasks.filter(t => t.cat === 'personal');
  ...
  if (!list.length) {
    el.innerHTML = '<div class="empty">No hay tareas aqui todavia.</div>';
    return;
  }
  el.innerHTML = list.map(t => {
    const ds = dateStatus(t.deadline);
    const dateHtml = t.deadline
      ? `<span class="date-tag ${ds}">${ds === 'vencido' ? 'Vencido' : 'Limite'}: ${fmt(t.deadline)}</span>`
      : '';
    ...
    return `
      <div class="task-card ${t.status === 'completada' ? 'done' : ''}">
        <div class="task-check ${t.status === 'completada' ? 'checked' : ''}"
             onclick="toggleDone(${t.id})" role="checkbox"
             aria-checked="${t.status === 'completada'}" aria-label="Marcar como completada">
          ${t.status === 'completada' ? '✓' : ''}
        </div>
        <div class="task-body">
...
function buildReport() {
  ...
  const shifts  = tasks
    ...
    .map(t => Math.round((new Date(t.completedAt) - new Date(t.doDate)) / (1000 * 60 * 60 * 24)));
  const avgShift = shifts.length ? Math.round(shifts.reduce((a, b) => a + b, 0) / shifts.length) : 0;
...
function renderReport() {
  ...
  document.getElementById('informe-content').innerHTML = `
    <div class="stat-grid">
      <div class="stat-box"><div class="stat-label">Completadas</div><div class="stat-value green">${r.done}</div></div>
      <div class="stat-box"><div class="stat-label">Pendientes</div><div class="stat-value">${r.pendingCount}</div></div>
      <div class="stat-box"><div class="stat-label">Vencidas sin completar</div><div class="stat-value red">${r.overdue}</div></div>
      <div class="stat-box"><div class="stat-label">Corrimiento promedio</div><div class="stat-value amber">${r.avgShift} dias</div></div>
    </div>
    ${r.pending.length ? `
    ...
    <ul class="informe-list">
      ${r.pending.map(t => `<li>${t.title} - <strong>${t.cat}</strong>${t.deadline ? ' · limite ' + fmt(t.deadline) : ''}</li>`).join('')}
